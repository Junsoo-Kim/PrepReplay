...
### PR #10 — 배치 처리 (`feat/batch-processing`)
`prepreplay run`에 영상 파일 대신 디렉터리를 넘기면 그 안의 지원 확장자 영상을 파일명순으로
일괄 처리한다. 영상 하나가 실패해도 배치 전체를 멈추지 않고 계속 진행하며, 끝에 성공/실패
요약 표를 보여준다(하나라도 실패하면 종료 코드 1). 기존 단일 파일 입력 경로는 완전히 동일하게
동작한다(`run()`의 단일 영상 처리 로직을 `_run_single_video()`로 추출해 재사용, 기존 테스트
106개 전부 무수정 통과). **기획서 마일스톤 5단계(선택 확장) 착수, `v1.1.0` 태그.**

> 설계 포인트: PR #9의 `state.json`/`run.log`가 이미 영상별(`output/{video_name}/`)로
> 분리돼 있던 덕분에, "배치 도중 중단되거나 일부만 실패해도 재실행 시 이미 끝난 영상은
